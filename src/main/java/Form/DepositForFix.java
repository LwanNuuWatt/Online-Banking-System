...
public class DepositForFix extends JFrame{
    ...
    public DepositForFix(String PhoneNo) {
        ...
        SwingUtilities.invokeLater(() ->{
            ...
            horizonLabel.setBounds(75, 140, 150, 30);
            ...
            depositLabel.setBounds(75, 180, 150, 20);
            leftPanel.add(depositLabel);

            // Right panel for the deposit form
            JPanel rightPanel = new JPanel();
            rightPanel.setBounds(300, 0, 600, 450);
            ...
            depositTitleLabel.setBounds(180, 30, 150, 30);
            ...
            ActionListener submit = (_ ->{
                ...
                {
                    JOptionPane.showMessageDialog(DepositForFix.this,
                ...
                try{
                    ...
                    if(result)
                    {
                        ...
                        transaction.setTotal_amount(amount.add(user.getAmount()));
                        transaction.setTenure_years(tenureYear);
                        transaction.setCompound_frequency(12);

                        boolean res = userController.setTransaction(transaction);
                        userController.setFixedDeposit(transaction);
                        userController.updateFixedDeposit(transactionID);
                        String message = res ? "Transaction saved!" : "An error occurred when saving transaction!";
                        JOptionPane.showMessageDialog(DepositForFix.this, message,
                                "Message",JOptionPane.INFORMATION_MESSAGE);
                        new customerSide(PhoneNo);
                        dispose();
                    }
    ...
    private void showTenureYearDialog() {
        String input = JOptionPane.showInputDialog(
                null,
                "Enter the tenure year for your Fixed Deposit:",
                "Tenure Year",
                JOptionPane.QUESTION_MESSAGE);

        try {
            if (input != null && !input.trim().isEmpty()) {
                tenureYear = Integer.parseInt(input.trim());
                JOptionPane.showMessageDialog(null, "You have selected a tenure of: "
                        + tenureYear + " year(s).");
            } else {
            ...
            JOptionPane.showMessageDialog(null, "Please enter a valid number for tenure year.");
            showTenureYearDialog();
        }
    }
}
